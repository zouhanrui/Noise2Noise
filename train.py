...
class AugmentGaussian:
    ...
    def add_train_noise_tf(self, x):
        ...
        #rng_stddev = tf.constant(1/255.0, dtype = "float32", shape=[1,1,1])
...
def train(
    ...
    # The only particular scenario in which ”feed_dict“ mechanism is to be used is during inferencing of data during deployment.
    # The replacement of feed_dict has taken place with Dataset and Iterator
    # Here we use Iterator.
    ...
    with tf.name_scope('Inputs'), tf.device("/cpu:0"):
        ...
        #
        # get_next function of iterator is used to create an operation in your Tensorflow graph which when run over a session, 
        # returns the values from the fed Dataset of iterator
        # which returns 3 Tensors with shape (?,3,256,256): NOISED data, NOISED data, CLEANED data
        #
        noisy_input, noisy_target, clean_target = dataset_iter.get_next()
        #noisy_input: <tf.Tensor 'Inputs/split:0' shape=(?, 3, 256, 256) dtype=float32>
        noisy_input_split = tf.split(noisy_input, submit_config.num_gpus)
        #After spliting : [<tf.Tensor 'Inputs/split:0' shape=(?, 3, 256, 256) dtype=float32>, 
        #                  <tf.Tensor 'Inputs/split:1' shape=(?, 3, 256, 256) dtype=float32>]
    ...
    for i in range(iteration_count):
        ...
        if i % eval_interval == 0:
            ...
            #check the current net trained so far
            denoised = net.run(source_mb)
            
            #this is the prediction for current trained net from training dataset, print every 1000-loop
            ...
            #Validate thru 24 images using the current model learned after ith iterations.
            #Calculate the average prediction's PSNR
